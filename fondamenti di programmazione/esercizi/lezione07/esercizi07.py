...
def search_word(filename: str, word: str) -> int:
    lowered_string = load_file(filename).lower() 
    for i in '!?,;.:-_"\'\n\t':
        lowered_string = lowered_string.replace(i, ' ')
    return lowered_string.split(' ').count(word)
...
def load_table(filename: str) -> List[Dict[str, str]]:
    entries = [entry.split() for entry in load_file(filename).split('\n') if entry != '']
    keys = entries[0]
    entries = entries[1:]
    return [{key : value for key, value in zip(keys, entry)} for entry in entries]
...
def load_data(filename: str) -> List[Dict[str, Union[int, str]]]:
    ...
    keys = entries[0]
    entries = entries[1:]
    return [{key : dynamic_int(value) for key, value in zip(keys, entry)} for entry in entries]
...
def load_csv(filename: str, empties: bool) -> List[Dict[str, Union[int, str, None]]]:
    ...
    if not empties:
        return [{key : dynamic_int(value) for key, value in zip(keys, entry) if value != ''} for entry in entries]
    else:
        return [{key : dynamic_int(value) for key, value in zip(keys, entry)} for entry in entries]
...
def column_stats(filename: str) -> Dict[str, Tuple[str, int]]:
    ...
    people = load_csv(filename, False)

    spese = sum([person['Spese'] for person in people if 'Spese' in person.keys()])
    tasse = sum([person['Tasse'] for person in people if 'Tasse' in person.keys()])

    return {'Spese': spese, 'Tasse': tasse}
