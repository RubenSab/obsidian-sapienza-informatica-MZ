...
def check_S_in_T(S, T):
    ...
    numbers_to_check = list(S)
    return [check_int_str(T, i) for i in numbers_to_check]
...
def list_multipli(L):
    m2 = [i for i in L if i%2==0]
    m5 = [i for i in L if i%5==0]
    return m2+m5
...
def list_to_tuple(L):
    ...
    return tuple([list(reversed(list(i))) for i in L if product(i)%2==0])
...
def sort_by_month(L):
    ...
    return list(sorted(L, key = lambda x: mesi.index(x.lower())))
